...
export default function AddProjectModal({
  ...
  const handleSubmit = async (
    ...
    if (!title.trim())
      return showToast("Le titre est obligatoire");

    if (!desc.trim())
      return showToast("La description est obligatoire");

    if (!tech.trim())
      return showToast("Les technologies sont obligatoires");

    if (!features.trim())
      return showToast("Les fonctionnalités sont obligatoires");

    if (images.length === 0)
      return showToast("Importez au moins 1 image");
